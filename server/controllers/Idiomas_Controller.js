...
const Crear_Idioma = async (req, res) => {
    ...
    try {
        ...
        if (!idioma_t) {
            return res.status(409).json({ success: false, error: "Idioma duplicado" });
        }
        ...
        return res.status(200).json({ success: true, msg: 'Se ha creado el idioma exitosamente', idioma_id: idioma._id });
    } catch (error) {
...
const Obtener_Idioma = async (req, res) => {
	...
	try {
        ...
        if (!idioma) {
            return res.status(404).json({ success: false, error: "Idioma no encontrado" });
        }
...
const Obtener_Idiomas = async (req, res) => {
	try {
        return res.status(200).json({ success: true, msg: 'Se han encontrado los idiomas', idiomas_puesto: Idiomas.find()});
    } catch (error) {
        return res.status(500).json({ success: false, error: 'Error interno del servidor' });
    }
};
...
const Actualizar_Idioma = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ success: true, msg: 'Se ha actualizado el idioma exitosamente', idioma_id: idioma._id });
    } catch (error) {
        return res.status(500).json({ success: false, error: 'Error interno del servidor' });
    }
};
...
const Eliminar_Idioma = async (req, res) => {
    try {
        ...
        return res.status(200).json({ success: true, message: 'Idioma eliminado correctamente' });
    } catch (error) {
