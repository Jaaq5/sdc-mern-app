...
const Catgorias_Curriculum = require('../models/Catgorias_Curriculum_Model');
const Catgorias_Puesto = require('../models/Catgorias_Puesto_Model');
...
const Crear_Curriculum = async (req, res) => {
    ...
    try {
        ...
        if (!usuario) {
            return res.status(404).json({ success: false, error: "Usuario no encontrado al crear curriculum" });
        }
       ...
       await Bloque.save()
        ...
        return res.status(201).json({ success: true, msg: 'Se ha registrado el usuario exitosamente', usuario_id: user._id });
    } catch (error) {
		console.log(error)
...
const Actualizar_Usuario = async (req, res) => {
    ...
    try {
        const user = await Usuarios.findById(new ObjectId(usuario_id));
        if (!user) {
            return res.status(404).json({ success: false, error: 'No se encontró al usuario' });
        }
		...
		if (curriculums_ids)
			user.Curriculums_IDs = curriculums_ids;

        await user.save();

        return res.status(200).json({ success: true, msg: 'Se ha actualizado al usuario exitosamente', usuario_id: user._id });
    } catch (error) {
		console.log(error);
...
const Actualizar_Usuario_Bloque = async (req, res) => {
    ...
    try {
        ...
        const bloque_datos = await Bloques.findById(user.Bloque_ID);
        if (!bloque_datos) {
            return res.status(404).json({ success: false, error: 'No se encontró al bloque del usuario' });
        }
...
const Log_In = async (req, res) => {
    ...
    const usuario = await Usuarios.findOne({ Email: email, Contrasena: contrasena });
    let usuario_id;
    if (!usuario) {    
	...
	return res.status(200).json({ success: true, msg: 'Log in exitoso.', usuario_id: usuario_id });
};
...
const Obtener_Datos_Usuario = async (req, res) => {
    ...
    try {
		...
		const curriculums = await Curriculums.find({"_id": {"$in": usuario.Curriculums_IDs}});
		...
		return res.status(200).json({ success: true, msg: "Usuario encontrado", data: userData });
    } catch (error) {
		console.log(error.message)
        return res.status(400).json({ success: false, error: 'Error interno del servidor' });
    }
		
		
const Actualizar_Usuario_Curriculum = async (req, res) => {
	const {usuario_id, curriculum_id, documento, categoria_curriculum_id, categoria_puesto_id} = req.params;  try {
	...
	if(!user._id.equals(curriculum.ID_Usuario))
	...
	if (!cat_curr) {
		return res.status(404).json({ success: false, error: 'No se encontró la categoria de curriculum al actualizar curriculum' });
	}
	
	const cat_puesto = await Categorias_Puesto.findById(new ObjectId(categoria_puesto_id));
	if (!cat_puesto) {
		return res.status(404).json({ success: false, error: 'No se encontró la categoria de puesto al actualizar curriculum' });
	}
...
const Eliminar_Usuario_Curriculum = async (req, res) => {
	...
	console.log(usuario_id + ", " + curriculum_id);
    try {
		const usuario = await Usuarios.findById(new ObjectId(usuario_id));
		if (!usuario) {
		  return res
		...
		if(!usuario._id.equals(curriculum.ID_Usuario))
			return res.status(403).json({ success: false, error: "Curriculo sin acceso." });
		
		var nueva_lista = usuario.Curriculums_IDs.filter((id) => !id.equals(curriculum_id_v));
		usuario.Curriculums_IDs = nueva_lista;
		
        await usuario.save();
		awaitCurriculums.deleteOne({_id: curriculum_id_v});
		
        return res.status(200).json({ success: true, msg: "Curriculo eliminado exitosamente." });
    } catch (error) {
		console.log(error.message)
        return res.status(400).json({ success: false, error: 'Error interno del servidor' });
    }
...
const Eliminar_Usuario = async (req, res) => {
    ...
    try {
		...
		await Curriculums.deleteMany({ ID_Usuario: us_id });
