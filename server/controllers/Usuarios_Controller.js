...
const Crear_Usuario = async (req, res) => {
    ...
    try {
        ...
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
            return res.status(401).json({ success: false, error: 'Formato de correo electrónico inválido' });
        }
        

        const existingEmail = await Usuarios.findOne({ Email: email });
        if (existingEmail) {
            return res.status(402).json({ success: false, error: "Dirección de correo electrónico ya está en uso" });
        }
		
		const Bloque = new Bloques({Bloques: {
