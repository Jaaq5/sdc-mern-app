  ...
  readLine.question(
    ...
    (uri) => {
      if (uri.length <= 50) {
        ...
        console.log(`${RED}Database name is too long. Please enter a name up to 50 characters.${RESET}`);
...
const validateIp = () => {
    return new Promise((resolve) => {
      ...
      readLine.question("Please enter the public IP address for client: ", (ip) => {
        const ipRegex = /^([0-9]{1,3}\.){3}[0-9]{1,3}$/;
        if (ipRegex.test(ip)) {
          resolve(ip);
        } else {
          console.log(`${RED}Invalid IP address. Please enter a valid public IP address.${RESET}`);
...
const installDependencies = (directory) =>
  new Promise((resolve, reject) => {
    exec(`cd ${directory} && npm install --omit=dev`, (error) => {
      ...
      console.log(`${GREEN}Production dependencies installed successfully.${RESET}`);
      resolve();
    });
  });
...
const setupEnvFile = (envFile, exampleEnv, replacements) => {
  ...
  if (!fs.existsSync(envFile)) {
    ...
    if (fs.existsSync(exampleEnv)) {
      ...
      console.log(`${GREEN}.env.production setup successfully for server.${RESET}`);
    } else {
      console.log(`${RED}The example .env file does not exist: ${exampleEnv}${RESET}`);
      process.exit(1);
    }
  } else {
    console.log(`${BLUE}.env.production already exists. Skipping creation.${RESET}`);
  }
};
...
const setupServerEnv = (port, mongoUri) => {
  ...
  setupEnvFile(envFile, exampleEnv, [
    ...
    ["MONGO_URI=CHANGEMYNAME", `MONGO_URI=mongodb://localhost:27017/${mongoUri}`],
  ]);
};
...
const setupClientEnv = async (port) => {
    ...
    if (!fs.existsSync(envFile)) {
      ...
      if (fs.existsSync(exampleEnv)) {
        ...
        try {
          ...
          content = content
            ...
            .replace(/REACT_APP_API_URL=CHANGEMYNAME/, `REACT_APP_API_URL=http://${publicIp}:${port}`);
  
          fs.writeFileSync(envFile, content, "utf8");
          console.log(`${GREEN}.env.production setup successfully for client.${RESET}`);
        } catch (error) {
          console.error(`${RED}Error setting up .env.production for client: ${error.message}${RESET}`);
          process.exit(1);
        }
      } else {
        console.log(`${RED}The example .env file does not exist: ${exampleEnv}${RESET}`);
      ...
      console.log(`${BLUE}.env.production already exists in client. Skipping creation.${RESET}`);
    }
  };
