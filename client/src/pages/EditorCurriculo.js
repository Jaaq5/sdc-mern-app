...
import {
  mapListaToHTML,
  SeccionOrderEditor,
} from "../Components/Editor/ListaOrden";
...
import {
  ...
  resolucionCeldas,
  GridCSS,
} from "../Components/Editor/EditorTamano";
...
import { listButtonStyle } from "../style";
import { theme } from "../theme";

const ToolBoxSwitcher = ({
  user_data,
  TextoEditar,
  setTextoEditar,
  ListaEditar,
...
function EditorCurriculo({
  ...
  const tamanoYPosicion = (estructura, pindex = 0, index = 0) => {
    ...
    if (estructura.Pos) {
      ...
      minMaxDoc[2] =
        minMaxDoc[2] > t.width + tc[0] ? minMaxDoc[2] : t.width + tc[0];
      minMaxDoc[3] =
        minMaxDoc[3] > t.height + tc[1] ? minMaxDoc[3] : t.height + tc[1];
  ...
  const obtenerTextoEstructura = (
    ...
    estructura.Texto.forEach((campo, index) => {
      if (skipNext > 0) {
        ...
        texto += formatoYear(
          "__year",
          user_data.bloques[nombreSeccion][id][estructura.Texto[index + 1]],
        );
        skipNext = 1;
      } else if (campo === "__check") {
        texto += user_data.bloques[nombreSeccion][id][
          estructura.Texto[index + 2]
        ]
          ? estructura.Texto[index + 1] +
            user_data.bloques[nombreSeccion][id][estructura.Texto[index + 2]]
          : "";
        skipNext = 2;
      } else if (seccion[campo] || seccion[campo] === "") {
        //Titulo y otros de plantilla
        texto +=
          seccion[campo] === ""
            ? (estructura.Editable && estructura.Editable.Placeholder) || ""
            : seccion[campo];
      } else if (
        id &&
  ...
  const ElementoTextoEditableHTML = ({
    user_data,
    documento,
    nombreSeccion,
    seccion,
  ...
  const ElementoEditableHTML = ({
    ...
    if (!estructura || !estructura.Editable) return <></>;
    switch (estructura.Editable.Tipo) {
      case "Texto":
      case "Estilo":
        return (
          <>
            <ElementoTextoEditableHTML
              user_data={user_data}
              documento={documento}
              nombreSeccion={nombreSeccion}
              seccion={seccion}
    ...
    return (
      <>
        <div
          id={nid}
          style={estructura.style}
          key={nombreSeccion + id + path}
          onMouseEnter={(e) => {
            document.getElementById(nid).style.boxShadow =
              "inset 0 0 0 2px purple";
          }}
          onMouseLeave={(e) => {
            document.getElementById(nid).style.boxShadow =
              "inset 0 0 0 0px purple";
          }}
        >
          <BotonEditable
            estructura={estructura}
  ...
  const ElementoImgEstructuradoHTML = ({
    ...
    return (
      <div
        ...
        style={estructura.style}
        onMouseEnter={(e) => {
          document.getElementById("Container_" + nid).style.boxShadow =
            "inset 0 0 0 2px purple";
        }}
        onMouseLeave={(e) => {
          document.getElementById("Container_" + nid).style.boxShadow =
  ...
  const ElementoEstructuradoHTML = ({
    ...
    switch (estructura.Tipo) {
          ...
          <div id={eid} style={style}>
            ...
            {Object.entries(estructura.Estructura).map(([index, estr]) => (
              <ElementoEstructuradoHTML
                ...
                id={id}
                index={index}
                path={extenderPath(path, ["Estructura", index])}
              />
            ))}
          </div>
        );
      case "IDs":
                ...
                return (
                  <>
                    <ElementoEstructuradoHTML
                      ...
                      id={bloque_id}
                      index={index}
                      path={extenderPath(path, ["Plantilla", index])}
                    />
                  </>
                );
              })}
        ...
        return (
          <div id={nid} style={estructura.style}>
            <BotonEditable
              estructura={estructura}
              seccion={seccion}
              nombreSeccion={nombreSeccion}
    ...
    if (documento.diseno.Secciones[estructura]) {
      return (
        <SeccionHTMLEstructurada
          ...
          path={["diseno", "Secciones", estructura]}
        />
      );
      ...
      return (
        <SeccionHTMLEstructurada
          ...
          path={[
            "diseno",
            "Secciones",
            documento.diseno.Secciones.Orden[estructura],
          ]}
        />
      );
      ...
      if (!estructura.Pos && ps[0] !== -1) {
        estructura.Pos = [
          (ps[0] - ps[2] + ps[3]) / celdasPagina[0],
          (ps[1] - ps[4] - ps[5]) / celdasPagina[1],
        ];
      }
      estructura.style = tamanoYPosicion(estructura, pindex, estructura);
      return (
        <div id={id} style={estructura.style}>
          <BotonEditable
            estructura={estructura}
            seccion={null}
            nombreSeccion={""}
              ...
              if (typeof estructura.Estructura[index] === "string")
                return (
                  <SeccionHTMLEstructurada
                    ...
                    documento={documento}
                    path={["diseno", "Secciones", estructura.Estructura[index]]}
                  />
                );
  ...
  const PaginaHTMLEstructurada = ({
    ...
    return (
      <div style={styled} id={"pagina_" + pindex}>
        <div style={{ position: "absolute" }}>
          {documento.diseno.Paginas[pindex]?.Elementos ? (
            Object.entries(documento.diseno.Paginas[pindex].Elementos).forEach(
              ([key, val]) => {
        ...
        {documento.diseno.Paginas[pindex]?.Estructura ? (
          Object.entries(documento.diseno.Paginas[pindex].Estructura).map(
            ([key, val]) => {
              return (
                <SubPaginaEstructuraHTML
                  user_data={user_data}
                  documento={documento}
                  estructura={val}
                  path={["diseno", "Paginas", pindex, "Estructura", key]}
                />
              );
            },
          )
  ...
  const SeleccionarIDs = (
    ...
    Object.keys(documento.datos.Secciones).map((seccion) => {
      let cantidad =
        ...
        documento.diseno.Secciones[seccion].Editable.Arreglo
          ? documento.datos.Secciones[seccion].Cantidad
          : 1;
      ...
      for (
        let i = 0;
        tempIds[seccion].length < cantidad && i < lista.length;
        i += 1
      ) {
        if (documento.diseno.Secciones[seccion].Editable?.Arreglo)
  ...
  const posicionEnOverlay = (id) => {
    ...
    pos = [
      pos[0] + doc.offsetLeft * 0 + container.offsetLeft - editor.scrollLeft,
      pos[1] + doc.offsetTop * 0 + container.offsetTop - editor.scrollTop,
    ];
    pos = [
      Math.max(-10000, pos[0]),
      Math.max(-10000, pos[1]),
  ...
  const MyHTMLDocument = () => {
    if (!documento) return <></>;
      ...
      Object.entries(documento.diseno.Secciones).forEach(
        ([nombreseccion, seccion]) => {
          ...
          if (!seccion.Pos && p[0] !== -1) {
            seccion.Pos = [
              (p[0] - p[2] + p[3]) / celdasPagina[0],
              (p[1] - p[4] - p[5]) / celdasPagina[1],
            ];
          }
          if (seccion.Celdas) {
          ...
          if (seccion.Estructura)
            Object.entries(seccion.Estructura).forEach(([index, estr]) => {
              let newStyle = {};
              Object.entries(estr.style ? estr.style : {}).forEach(
                ([key, val]) => (newStyle[key] = val),
              );
              estr.style = newStyle;
              if (estr.Celdas) {
              ...
              if (estr.Plantilla)
                Object.entries(estr.Plantilla).forEach(([index2, plnt]) => {
                  newStyle = {};
                  Object.entries(plnt.style ? plnt.style : {}).forEach(
                    ([key, val]) => (newStyle[key] = val),
                  );
                  plnt.style = newStyle;
                  if (plnt.Celdas) {
      ...
      const calcDivisor = (numeroDePaginas) => {
        let style = {};
        Object.entries(divisorPagina).forEach(([key, value]) => {
          style[key] = value;
        });
      ...
      let numeroDePaginas = pagina
        ? Math.ceil(height / (celdasPagina[1] * resolucionCeldas))
        : 1;

      const divisores = [];
      for (let i = 1; i < numeroDePaginas; i += 1)
        divisores.push(
          <div style={calcDivisor(i)} id={"divisor_pagina_" + (i + 1)}></div>,
        );

      Object.entries(documento.diseno.Paginas[0].style).forEach(
        ([key, value]) => {
          paginaEstilo[key] = value;
        },
      );
      paginaEstilo.height =
        numeroDePaginas * (celdasPagina[1] * resolucionCeldas) + "px"; //minMaxDoc[3]+"px"
      ...
      return (
        <div id="documento_html" style={documentoEstilo}>
  ...
  const handleWheel = (e, zoom, setZoom) => {
    if (e.ctrlKey) {
      e.preventDefault();
      if (!Editando) {
        controles.zoom = Math.min(
          Math.max(
            (controles.zoom ? controles.zoom : 1) +
  ...
  useEffect(() => {
    ...
    if (!user_data || !user_data.editando_curriculo) {
      ...
      //DEBUG
      let doc = user_data.curriculums[user_data.editando_curriculo].Documento
        ?.diseno?.__Comment
        ? user_data.curriculums[user_data.editando_curriculo].Documento
        : curriculum_manager.CopiarPlantilla("simple").Documento;
      doc =
        editMode === "Usuario"
          ? doc
          : curriculum_manager.CopiarPlantilla("simple").Documento;
  ...
  return (
    <>
      <div
        id={"pagina_medidor"}
        style={{ width: "594px", height: "841px", display: "none" }}
      >
        ...
        <div style={{ display: "flex", minHeight: "100%" }}>
          ...
          <div id={"Editor_Caja"} style={pdfCaja}>
            <div id={"Titulo_Editor"} style={stripeStyle}>
              <div style={textOverlayStyle}>Editor de dator del currículo</div>
            </div>
              ...
              {process.env.NODE_ENV === "development" ? (
                <>
                  <Button
                    ...
                    onClick={(e) => {
                      const edit =
                        editMode === "Usuario" ? "Plantilla" : "Usuario";
                      let doc = user_data.curriculums[
                        user_data.editando_curriculo
                      ].Documento?.diseno?.__Comment
                        ? user_data.curriculums[user_data.editando_curriculo]
                            .Documento
                        : curriculum_manager.CopiarPlantilla("simple")
                            .Documento;
                      doc =
                        edit === "Usuario"
                          ? doc
                  ...
                  {editMode === "Plantilla" ? (
                    <Button
                      ...
                      onClick={(e) => {
                        ...
                        const id =
                          documento.datos.Secciones.Informacion_Personal;
            ...
            {Editando ? (
              <div
                ...
                style={{
                  ...
                  zoom: 1,
                  pointerEvents:
                    Editando.Celdas || Editando.Pos ? "none" : "auto",
                }}
              >
                <ToolBoxSwitcher
                  user_data={user_data}
            ...
            {Editando ? (
              <div
                ...
                style={{
                  ...
                  zoom: zoom,
                  pointerEvents:
                    Editando.Celdas || Editando.Pos ? "auto" : "none",
                }}
              >
          ...
          >
            <div style={stripeStyle}>
              <div style={textOverlayStyle}>Vista previa</div>
            </div>

            <div style={{ flexGrow: 1 }}>
              {" "}
              {/* Make this div grow to take available space */}
              {!Editando ? (
