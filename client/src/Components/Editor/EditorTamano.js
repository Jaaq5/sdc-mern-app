...
const tamanoObjeto = (path, documento, setDocumento, id = "") => {
  ...
  if (elm) {
    ...
    mrgnCeldas[0] =
      Number(style.marginLeft.substring(0, style.marginLeft.length - 2)) /
      celdasPagina[0];
    mrgnCeldas[1] =
...
const calcularBotones = (
  ...
  for (let i = 0; i < posiciones.length; i += 1) {
    visualPosiciones.push([
      posiciones[i][0] * tamano.width,
      posiciones[i][1] * tamano.height,
    ]);
    movimiento.push([0, 0]);
    origenes.push(0);
  }
  ...
  const drag = (accion, index) => {
    origenes[index] =
      origenes[index] === 0
        ? [accion.clientX / gzoom, accion.clientY / gzoom]
        : origenes[index];
  };
  ...
  const updatePos = (accion, myindex, posiciones, cajax) => {
    movimiento[myindex] =
      origenes[myindex] === 0
        ? movimiento[myindex]
        : [
            accion.clientX / gzoom - origenes[myindex][0],
            accion.clientY / gzoom - origenes[myindex][1],
          ];
    movimiento[myindex] = [
      movimiento[myindex][0] * mults[myindex][0],
      movimiento[myindex][1] * mults[myindex][1],
    ];
    if (origenes[myindex] !== 0) {
      accion.target.style.transition = "none";
      accion.target.style.transform =
        "translate(" +
        movimiento[myindex][0] +
        "px," +
        movimiento[myindex][1] +
        "px)";
      accion.target.style.minWidth = size * 100 + "px";
      accion.target.style.minHeight = size * 100 + "px";
      accion.target.style.margin =
        -size * 50 + "px 0px 0px " + -size * 50 + "px";

      let t = [
        Math.min(Math.max(tamano.width + movimiento[myindex][0], 5), 2756),
        Math.min(Math.max(tamano.height + movimiento[myindex][1], 5), 21123),
      ];
      const c = [
        Math.floor(t[0] / celdasPagina[0]),
        Math.floor(t[1] / celdasPagina[1]),
      ];
      ...
      document.getElementById("tamano_celda_texto").innerHTML =
        c[0] + " x " + c[1]; //Math.floor(t[0])+" x "+Math.floor(t[1]);
  ...
  const end = (accion, index, path, documento, setDocumento, cajax) => {
    let t = [
      Math.min(
        Math.max(tamano.width + movimiento[index][0], celdasPagina[0]),
        2756,
      ),
      Math.min(
        Math.max(tamano.height + movimiento[index][1], celdasPagina[1]),
        21123,
      ),
    ];
    let item = documento;
...
const calcularBotonMovimiento = (
  ...
  const drag = (accion, index) => {
    ...
    scrollOrigin =
      origen === 0
        ? [window.scrollX / gzoom, window.scrollY / gzoom]
        : scrollOrigin;
    origen =
      origen === 0
        ? [accion.clientX / gzoom, accion.clientY / gzoom]
        : [
            origen[0] - (ended[0] - accion.clientX / gzoom),
            origen[1] - (ended[1] - accion.clientY / gzoom),
          ];
    direction = [true, true];
  ...
  const updatePos = (accion, id) => {
    movimiento = !dragging
      ? movimiento
      : [
          accion.clientX / gzoom -
            (scrollOrigin[0] - window.scrollX / gzoom) * 1 -
            origen[0],
          accion.clientY / gzoom -
            (scrollOrigin[1] - window.scrollY / gzoom) * 1 -
            origen[1],
        ];
    ...
    if (
      ...
      direction[0] &&
      direction[1] &&
      Math.abs(movimiento[0] * movimiento[1]) > 15
    ) {
      direction =
        Math.abs(movimiento[0]) > Math.abs(movimiento[1])
          ? [true, false]
          : [false, true];
    }
    ...
    if (dragging) {
      ...
      t = [
        Math.floor(movimiento[0] / celdasPagina[0]) * celdasPagina[0],
        Math.floor(movimiento[1] / celdasPagina[1]) * celdasPagina[1],
      ];
      ...
      if (accion.target.id === "botton_mover") {
        ...
        accion.target.parentElement.parentElement.style.transform =
          "translate(" + t[0] + "px," + t[1] + "px)";
      }

      document.getElementById("pos_celda_texto").innerHTML =
        c[0] + " x " + c[1];
    }
  };
  ...
  buttonm = (
    <button
      ...
      style={{
        backgroundColor: "#0000",
        border: 0,
        minWidth: tamano.width + "px",
        minHeight: tamano.height + "px",
        position: "absolute",
        padding: 0,
        display: "flex",
        justifyContent: "center",
...
function manejarBorrarSeccion(Editando, documento, setDocumento) {
  if (Editando.Seccion) {
    const index = documento.diseno.Paginas[0].Estructura.indexOf(
      Editando.Seccion,
    );
...
function manejarOrdenCapa(dir, Editando, documento, setDocumento) {
  ...
  if (
    dir === "up" &&
    index < documento.diseno.Paginas[0].Estructura.length - 1
  ) {
    documento.diseno.Paginas[0].Estructura[index] =
      documento.diseno.Paginas[0].Estructura[index + 1];
...
function OldGridCSS({}) {
  const gridStyle = {
    ...
    position: "absolute",
    backgroundImage:
      "linear-gradient(#fcc3 0 1px, transparent 1px 100%), linear-gradient(90deg, #ccf3 0 1px, transparent 1px 100%)",
...
const EditorTamano = ({
  ...
  if (!Editando || !Editando.path || !Editando.id) return <></>;
  let e = false;
  const cajax = document.getElementById(Editando.id);
  if (err === null) {
    e =
      cajax?.clientHeight < cajax?.scrollHeight ||
      cajax?.clientWidth < cajax?.scrollWidth;
    setErr(e);
  }
  ...
  if (tamano[0] === 0) setTamano(t);

  const m = cajax.currentStyle || window.getComputedStyle(cajax);
  const doc = document.getElementById("contenedor_documento");

  const offset = hasParent
    ? [0, 0]
    : [
        Math.floor(
          (Editando.pos[0] -
            Number(m.marginLeft.substring(0, m.marginLeft.length - 2)) -
            Editando.pos[2] +
            Editando.pos[3] * 1 +
            Number(doc.style.left.substring(0, doc.style.left.length - 2)) -
            Number(doc.style.left.substring(0, doc.style.left.length - 2)) *
              0) /
            celdasPagina[0],
        ),
        Math.floor(
          (Editando.pos[1] -
            Number(m.marginTop.substring(0, m.marginTop.length - 2)) +
            Editando.pos[4] * 0 +
            Editando.pos[5] * 1 -
            Number(doc.style.top.substring(0, doc.style.top.length - 2))) /
            celdasPagina[1],
        ),
      ];
  if (!item) item = { Pos: [0, 0], Celdas: [0, 0] };
  let initialPos = item.Pos ? item.Pos : [offset[0], offset[1]];

  const p = posicionObjeto(initialPos);
  if (pos == null) setPos(p);
  ...
  if (!botonMovimiento) calcularBotonMovimiento(...dataMov);
  ...
  return (
    <>
      <div
        ...
        style={{
          pointerEvents: "auto",
          width: cajax.clientWidth,
          height: cajax.clientHeight,
          backgroundColor: "#e495e800",
          border: "solid 3px #e495e8",
          borderRadius: "0px",
          position: "absolute",
          left: xy[0] + "px",
          top: xy[1] + "px",
      ...
      >
        <div
          style={{
            position: "absolute",
            top: Editando.pos[1] < 0 ? -Editando.pos[1] + "px" : "0px",
            width: "100%",
            minWidth: "270px",
            zoom: 1 / gzoom,
          }}
        >
            ...
            {Editando.Pos ? (
              <>
                <ControlCameraIcon
                  style={{ position: "relative", top: "5px" }}
                />{" "}
                <span id="pos_celda_texto">
                  {initialPos ? (
                    <span>
                      {Math.floor(initialPos[0]) + " "}x
                      {" " + Math.floor(initialPos[1])}
                    </span>
                  ) : (
                    <></>
                  )}
            ...
            {Editando.Celdas ? (
              <>
                <PhotoSizeSelectSmallIcon
                  style={{ position: "relative", top: "5px" }}
                />{" "}
                <span id="tamano_celda_texto">
                  {Math.floor(item.Celdas[0]) + " "}x
                  {" " + Math.floor(item.Celdas[1])}
                </span>
              </>
            ) : (
              <></>
          ...
          >
            <Button
              ...
              onClick={(e) => {
                manejarOrdenCapa("up", Editando, documento, setDocumento);
              }}
            >
              <KeyboardArrowUpIcon color="primary" />{" "}
            </Button>
            <span id="zIndex_layer">
              {documento.diseno.Paginas[0].Estructura.indexOf(Editando.Seccion)}
            </span>
            <Button
              style={{
                maxWidth: "50px",
              ...
              onClick={(e) => {
                manejarOrdenCapa("down", Editando, documento, setDocumento);
              }}
            >
            ...
            {Editando.Borrar ? (
              <Button
                ...
                onClick={(e) => {
                  manejarBorrarSeccion(Editando, documento, setDocumento);
                  setEditando(null);
                }}
              >
