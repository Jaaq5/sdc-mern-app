...
function obtenerOpcionesDeEstilos(documento, path) {
  const opciones = {};
  if (!documento || !path) return opciones;

  let item = documento;
  path.forEach((p) => {
    item = item[p] ? item[p] : item;
  });
  if (!item) return opciones;

  Object.entries(item.style ? item.style : {}).forEach(
    ([k, v]) => (opciones[k] = v),
  );
  opciones.fontSize = opciones.fontSize ? opciones.fontSize : "10px";
  opciones.fSize = Number(
    opciones.fontSize.substring(0, opciones.fontSize.length - 2),
  );
  opciones.fontFamily = opciones.fontFamily ? opciones.fontFamily : "Arial";
  opciones.fFamily = fontsData.fonts.find(
    (f) => f.fontFamily === opciones.fontFamily,
  ).name;
  opciones.fIndex = fontsData.fonts.indexOf(
    fontsData.fonts.find((f) => f.fontFamily === opciones.fontFamily),
  );

  opciones.color = opciones.color ? opciones.color : "#000000";
  opciones.backgroundColor = opciones.backgroundColor
    ? opciones.backgroundColor
    : "#FFFFFF00";
  opciones.borderColor = opciones.borderColor
    ? opciones.borderColor
    : "#FFFFFF";
  opciones.fontStyle = fontsData.fonts[
    opciones.fIndex
  ].importAvailability.includes("Italic")
    ? opciones.fontStyle
    : "normal";
  opciones.borderNums = {
    sizeTop: opciones.borderTopWidth
      ? Number(
          opciones.borderTopWidth.substring(
            0,
            opciones.borderTopWidth.length - 2,
          ),
        )
      : 0,
    sizeBottom: opciones.borderBottomWidth
      ? Number(
          opciones.borderBottomWidth.substring(
            0,
            opciones.borderBottomWidth.length - 2,
          ),
        )
      : 0,
    sizeLeft: opciones.borderLeftWidth
      ? Number(
          opciones.borderLeftWidth.substring(
            0,
            opciones.borderLeftWidth.length - 2,
          ),
        )
      : 0,
    sizeRight: opciones.borderRightWidth
      ? Number(
          opciones.borderRightWidth.substring(
            0,
            opciones.borderRightWidth.length - 2,
          ),
        )
      : 0,
    sizeAll: 1,
...
function manejarFontFamily(estado, val, fIndex) {
  ...
  estado.fontStyle = fontsData.fonts[fIndex].importAvailability.includes(
    "Italic",
  )
    ? estado.fontStyle
    : "normal";
  estado.fontWeight = fontsData.fonts[fIndex].importAvailability.includes(
    "Bold",
  )
    ? estado.fontWeight
    : 400;
  ...
  estado.fontWeight = fontsData.fonts[fIndex].importAvailability.includes(
    ...
    ? estado.fontWeight
    : Math.max(estado.fontWeight, 400);
  return estado;
}
...
function manejarBorde(estado, data) {
  ...
  switch (data.mode) {
    ...
    case "Right":
      data.size =
        data.size === estado["border" + data.mode + "Width"]
          ? "0pt"
          : data.size;
...
function actualizarBoton(id, estado, campo, def) {
  ...
  if (elm) {
    const s =
      estado[campo] === def
        ? ToolButtonStyle.backgroundColor
        : ToolButtonActiveStyle.backgroundColor;
...
function HerramientasElementos({
  opciones,
  documento,
  setDocumento,
  Editando,
  ...
  const mapDBListToHTML = (setter, lista) => {
    ...
    lista.forEach((item) => {
      ...
      s.fontWeight = 400;
      s.fontWeight = item.importAvailability.includes("Bold")
        ? s.fontWeight
        : 400;
      s.fontWeight = item.importAvailability.includes("Regular")
        ? s.fontWeight
        : 300;
      s.fontWeight = item.importAvailability.includes("Light")
        ? s.fontWeight
        : 400;
      subStyles.push(s);
    });
  ...
  return (
    <>
      ...
      >
        ...
        {Editando.Fuentes["color"] ? (
          <div style={ToolElmStyle}>
            <Button
              id={"font_edit_color_button"}
              style={ToolButtonStyle}
              onClick={(e) => {
                manejarExpandirOpciones(
                  "color_font_swatches",
                  "350px",
                  "350px",
                );
            ...
            >
              ...
              <div
                ...
                style={{
                  ...
                  width: "22px",
                  height: "4px",
                  backgroundColor: estadoOpciones.color,
                }}
              ></div>
            </Button>
            <div
              id={"color_font_swatches"}
              style={{
                overflow: "hidden",
                display: "flex",
                position: "absolute",
            ...
            >
              <SketchPicker
                ...
                onChange={(c, e) => {
                  ...
                  if (elm) elm.style.backgroundColor = c.hex;
                  setEstadoOpciones(manejarFontColor(estadoOpciones, c.hex));
                  upd();
                }}
              />
            </div>
        ...
        {Editando.Fuentes["backgroundColor"] ? (
          <div style={ToolElmStyle}>
            <Button
              id={"font_edit_bgcolor_button"}
              style={ToolButtonStyle}
              onClick={(e) => {
            ...
            >
              ...
              <div
                ...
                style={{
                  ...
                  width: "22px",
                  height: "4px",
                  backgroundColor: estadoOpciones.backgroundColor,
                }}
              ></div>
            </Button>
            <div
              id={"color_bg_swatches"}
              style={{
                overflow: "hidden",
                display: "flex",
                position: "absolute",
            ...
            >
              <SketchPicker
                ...
                onChange={(c, e) => {
                  ...
                  const alpha = Math.floor(c.rgb.a * 255)
                    .toString(16)
                    .toUpperCase();
        ...
        {Editando.Fuentes["border"] ? (
          <div style={ToolElmStyle}>
            <Button
              id={"border_edit_button"}
              style={ToolButtonStyle}
              onClick={(e) => {
            ...
            >
              ...
              <div
                ...
                style={{
                  ...
                  width: "22px",
                  height: "4px",
                  backgroundColor: estadoOpciones.borderColor,
                }}
              ></div>
            </Button>
            <div
            ...
            >
              <div>
                <div style={ToolElmStyle}>
                  <Button
                    ...
                    style={ToolButtonStyle}
                    onClick={(e) => {
                      const res = manejarBorde(estadoOpciones, {
                        mode: "Clear",
                      });
                ...
                <div style={ToolElmStyle}>
                  ...
                  <input
                    ...
                    aria-label="size_border_all"
                    placeholder="1"
                    defaultValue={border.sizeAll}
                    onChange={(event) => {
                      border.sizeAll = event.target.value;
                    }}
                  />
                </div>
                <div style={ToolElmStyle}>
                  ...
                  <input
                    ...
                    step="0.25"
                    style={{ width: "50px", height: "80%" }}
                    aria-label="size_border_top"
                    placeholder="1"
                    defaultValue={border.sizeTop}
                    onChange={(event) => {
                      border.sizeTop = event.target.value;
                    }}
                  />
                </div>
                <div style={ToolElmStyle}>
                  ...
                  <input
                    ...
                    step="0.25"
                    style={{ width: "50px", height: "80%" }}
                    aria-label="size_border_bottom"
                    placeholder="1"
                    defaultValue={border.sizeBottom}
                    onChange={(event) => {
                      border.sizeBottom = event.target.value;
                    }}
                  />
                </div>
                <div style={ToolElmStyle}>
                  ...
                  <input
                    ...
                    step="0.25"
                    style={{ width: "50px", height: "80%" }}
                    aria-label="size_border_left"
                    placeholder="10"
                    defaultValue={border.sizeLeft}
                    onChange={(event) => {
                      border.sizeLeft = event.target.value;
                    }}
                  />
                </div>
                <div style={ToolElmStyle}>
                  ...
                  <input
                    ...
                    step="0.25"
                    style={{ width: "50px", height: "80%" }}
                    aria-label="size_border_right"
                    placeholder="10"
                    defaultValue={border.sizeRight}
                    onChange={(event) => {
                      border.sizeRight = event.target.value;
                    }}
                  />
                </div>
              </div>
              <SketchPicker
                ...
                onChange={(c, e) => {
                  ...
                  if (elm) elm.style.borderColor = c.hex;
                  const res = manejarBorde(estadoOpciones, {
                    mode: "Color",
                    hcolor: c.hex,
                    side: border.lastSide,
                  });
                  setEstadoOpciones(res);
                  upd();
                }}
        ...
        {fontsData.fonts[fontIndex].importAvailability.includes("Bold") &&
        Editando.Fuentes["fontWeight"] ? (
          <div style={ToolElmStyle}>
            <Button
              id={"font_edit_bold_button"}
              style={
                estadoOpciones.fontWeight === 400
                  ? ToolButtonStyle
        ...
        {fontsData.fonts[fontIndex].importAvailability.includes("Italic") &&
        Editando.Fuentes["fontStyle"] ? (
          <div style={ToolElmStyle}>
            <Button
              id={"font_edit_italic_button"}
              style={
                estadoOpciones.fontStyle !== "italic"
                  ? ToolButtonStyle
                  : ToolButtonActiveStyle
              }
              onClick={(e) => {
        ...
        {Editando.Fuentes["textDecoration"] ? (
          <div style={ToolElmStyle}>
            <Button
              id={"font_edit_underline_button"}
              style={
                !estadoOpciones.textDecoration
                  ? ToolButtonStyle
                  : ToolButtonActiveStyle
              }
              onClick={(e) => {
          ...
          <div style={ToolElmStyle}>
            ...
            <input
              ...
              onChange={(event) => {
                ...
                setEstadoOpciones(
                  manejarFontSize(estadoOpciones, event.target.value),
                );
                upd();
              }}
            />
            ...
            >
              ...
              <NativeSelect
                ...
                onChange={(e) => {
                  ...
                  const i = fontsData.fonts.indexOf(
                    fontsData.fonts.find((f) => f.name === e.target.value),
                  );
                  setFontIndex(i);
                  setEstadoOpciones(
                    manejarFontFamily(estadoOpciones, e.target.value, i),
                  );
                  upd();
                }}
              >
              ...
              {Editando.Fuentes["justifyContent"] ? (
                <div
                  style={{
                    display: "flex",
                    flexDirection: "row",
                    height: ToolButtonStyle.maxWidth,
                  }}
                >
                  <div style={ToolElmStyle}>
                    <Button
                      id={"left_align_button"}
                      style={ToolButtonStyle}
                      onClick={(e) => {
