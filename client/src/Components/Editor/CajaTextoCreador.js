...
function manejarCreacionCaja(documento, setDocumento){
	documento.diseno.numCajas = documento.diseno.numCajas? documento.diseno.numCajas : 0;
	documento.diseno.numCajas += 1;
	const seccionName = plantilla_caja.Nombre.replace("<num>",""+documento.diseno.numCajas);
...
function CajaTextoCreador({documento, setDocumento, opciones, style, iconStyle, id}){
	return (
		<Button id={id} style={style} onClick={(e) => manejarCreacionCaja(documento, setDocumento)} title={"Nueva caja de texto"}>
