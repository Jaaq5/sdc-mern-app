...
const calcularBotonMovimiento = (pos, setBotonMovimiento, id) => {
  ...
  const drag = (accion) => {
    scrollOrigin =
      origen === 0 ? [window.scrollX, window.scrollY] : scrollOrigin;
    origen =
      origen === 0
        ? [accion.clientX, accion.clientY]
        : [
            origen[0] - (ended[0] - accion.clientX),
            origen[1] - (ended[1] - accion.clientY),
          ];
  ...
  const updatePos = (accion, id) => {
    movimiento = !dragging
      ? movimiento
      : [
          accion.clientX - (scrollOrigin[0] - window.scrollX) * 1 - origen[0],
          accion.clientY - (scrollOrigin[1] - window.scrollY) * 1 - origen[1],
        ];
    if (dragging) {
...
const DragButton = ({ id }) => {
  ...
  if (!botonMovimiento) calcularBotonMovimiento([0, 0], setBotonMovimiento, id);
