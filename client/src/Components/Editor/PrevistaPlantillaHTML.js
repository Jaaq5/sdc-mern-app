...
const ElementoTextoEstructuradoHTML = ({documento, nombreSeccion, seccion, estructura, id, index, obtenerTextoEstructura}) => {
  return (<>
...
const ElementoDivEstructuradoHTML = ({documento, nombreSeccion, seccion, estructura, id, index, obtenerTextoEstructura}) => {
  return (<>
  <p id={"HTML_Texto_"+nombreSeccion+"_"+index} style={estructura.style} key={nombreSeccion+id+index} >
		{obtenerTextoEstructura(nombreSeccion, seccion, id, estructura, index)}
  </p></>);
...
const ElementoImgEstructuradoHTML = ({documento, nombreSeccion, seccion, estructura, id, index}) => {
	  return (<>
		<img id={"Imagen_"+nombreSeccion+"_"+index} style={estructura.style} key={nombreSeccion+id+index} src={"/default-user-image.webp"} />
	  </>);
  };

const ElementoEstructuradoHTML = ({documento, nombreSeccion, seccion, estructura, id, index, obtenerTextoEstructura}) => {
  ...
  switch(estructura.Tipo){
	  case "Texto":
		return (<ElementoTextoEstructuradoHTML  documento={documento} nombreSeccion={nombreSeccion} seccion={seccion} estructura={estructura} id={id} index={index} obtenerTextoEstructura={obtenerTextoEstructura} />);
	  case "Div":
		return (<ElementoDivEstructuradoHTML  documento={documento} nombreSeccion={nombreSeccion} seccion={seccion} estructura={estructura} id={id} index={index} obtenerTextoEstructura={obtenerTextoEstructura} />);
	  case "Imagen":
		...
		for(let i = 0; i < documento.datos.Secciones[nombreSeccion].Cantidad; i+=1){
			list.push(
				<div style={estructura.plantillaStyle}>{Object.keys(estructura.Plantilla).map((index) => {
					return (
						<ElementoEstructuradoHTML  documento={documento} nombreSeccion={nombreSeccion} seccion={seccion} estructura={estructura.Plantilla[index]} index={index} obtenerTextoEstructura={obtenerTextoEstructura} />
					)
				})}</div>
			);
		};
...
const SeccionHTMLEstructurada = ({seccion, documento, id, obtenerTextoEstructura}) => {
	  ...
	  return (
		<div id={"Seccion_" + seccion} style={documento.diseno.Secciones[seccion].style} key={seccion}>
			{Object.entries(documento.diseno.Secciones[seccion].Estructura).map(([index, estructura]) => {
				return (<>
					<ElementoEstructuradoHTML  documento={documento} nombreSeccion={seccion} seccion={documento.diseno.Secciones[seccion]} estructura={documento.diseno.Secciones[seccion].Estructura[index]} id={id} index={index} obtenerTextoEstructura={obtenerTextoEstructura} />
				</>)
			})}
		</div>
	  );
...
const SubPaginaEstructuraHTML = ({documento, estructura, obtenerTextoEstructura}) => {
    if(documento.diseno.Secciones[estructura]){
		return (<SeccionHTMLEstructurada  seccion={estructura} documento={documento} id={documento.datos.Secciones[estructura]} obtenerTextoEstructura={obtenerTextoEstructura} />);
		
	}else if(documento.diseno.Secciones.Orden[estructura]){
		return (<SeccionHTMLEstructurada  seccion={documento.diseno.Secciones.Orden[estructura]} documento={documento} obtenerTextoEstructura={obtenerTextoEstructura}/>);
		...
		return (<div style={estructura.style}>
			{estructura.Estructura? (Object.keys(estructura.Estructura).map((index) => {
				 if(typeof(estructura.Estructura[index]) === "string")
					return(<SeccionHTMLEstructurada  seccion={estructura.Estructura[index]} id={documento.datos.Secciones[estructura.Estructura[index]]} documento={documento} obtenerTextoEstructura={obtenerTextoEstructura}/>);
				 else if(typeof(estructura.Estructura[index]) === "number")
					 return(<SeccionHTMLEstructurada  seccion={documento.diseno.Secciones.Orden[estructura.Estructura[index]]} documento={documento} obtenerTextoEstructura={obtenerTextoEstructura}/>);
				 else{
					 return(<SubPaginaEstructuraHTML  documento={documento} estructura={estructura.Estructura[index]} obtenerTextoEstructura={obtenerTextoEstructura}/>);
				 }
			})
			)
			:
	  ...
	  return (<div style={documento.diseno.Paginas[0].style} id={"pagina_"+1}>
				...
				</div>
				{
					Object.entries(documento.diseno.Paginas[0].Estructura).map(([key, val]) => {
						return (<SubPaginaEstructuraHTML  documento={documento} estructura={val} obtenerTextoEstructura={obtenerTextoEstructura}/>)
					})
				}
			  </div>);
  };
...
const obtenerTextoEstructura = (nombreSeccion, seccion, id, estructura, index) => {
	let texto = "";
...
const PrevistaPlantillaHTML = ({documento}) => {
    if (!documento) 
        ...
        >
          <PaginaHTMLEstructurada  documento={documento}  obtenerTextoEstructura={obtenerTextoEstructura}/>
        </div>
	)};
 };
