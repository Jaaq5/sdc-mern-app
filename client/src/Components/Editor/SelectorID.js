...
const crearBloquesToHTML = (user_data, TextoEditar, setTextoEditar, ListaEditar, setListaEditar, documento, setDocumento, Editando, setEditando) => {
	...
	let sortedBloques = OrdenarBloques(bloques, Editando.ID_Categoria_Curriculum, Editando.ID_Categoria_Puesto);//Object.entries(bloques).sort();
...
const SelectorID = ({user_data, ListaEditar, setListaEditar, documento, setDocumento, Editando, setEditando, SeleccionarIDs, zoom}) => {
	...
	return (
	<div id={"Selector_IDs"} style={{pointerEvents: "auto", backgroundColor: "#303030", border:"solid 0px #333", borderRadius: "0px", position: "absolute", left: (Math.max(Editando.pos[0]*0-300/zoom, 0)*zoom)+"px",top: (Math.max(Editando.pos[1]*0,20)*zoom)+"px", marginTop: "-10px", maxWidth: "300px", zIndex: 1000}}>
			<div style={{color: "#fff", fontWeight: "900", fontSize: "1.2em", borderBottom: "solid 2px rgb(200,200,200)", padding: "3px", textAlign: "center"}}>
				...
				<Button onClick={(e) => {setEditando(null); SeleccionarIDs(user_data, documento, user_data.curriculums[user_data.editando_curriculo].ID_Categoria_Curriculum, user_data.curriculums[user_data.editando_curriculo].ID_Categoria_Puesto);}}><CheckCircleIcon color = "success" /> </Button>
			</div>
