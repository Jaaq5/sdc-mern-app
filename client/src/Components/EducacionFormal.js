...
function EducacionFormal({ user_data, setUserData, manager_bloques, category_manager }) {
  ...
  const mapToHTML = (bloques) => {
    ...
    setEducaciones(
      Object.keys(bloques).map((plan_id, index) => (
        <ListItemButton key={plan_id} style={listStyle} onClick={(e) => editarDatos(plan_id)}>
          <ListItemText
            primary={
          ...
          />
		  <Button style={deleteButton} onClick={(e) => elminarBloque(plan_id, index)}><DeleteForever /></Button>
        </ListItemButton>
      ))
    );
  ...
  const mapDBListToHTML = (setter, lista) => {
      setter(Object.keys(lista).map((l_id) => (
  ...
  useEffect(() => {
    if (!user_data) {
	  ...
	  //Categorias
	  category_manager.ObtenerCategoriasCurriculum().then((response) => {
		  mapDBListToHTML(setCatCurr, response);
	  }).catch((e) => {});
	  
	  category_manager.ObtenerCategoriasPuesto().then((response) => {
  ...
  }, [user_data, setUserData, category_manager, navigate, setCatCurr, setCatPuesto, loading]); //Espera a que estos existan?
  ...
  const editarDatos = (plan_id) => {
	...
	setCurriculum(bloque.ID_Categoria_Curriculum? bloque.ID_Categoria_Curriculum : '');
  ...
  const manejarDatos = (e) => {
	...
	if (bloque_id !== true){
		manager_bloques.ActualizarBloque(user_data, setUserData, "Educacion_Formal", bloque_id, {
    ...
    //Crear Bloque Educacion Formal
		const bloque = manager_bloques.InsertarBloque(user_data, setUserData, "Educacion_Formal", {
		  Fecha_Inicio: fecha_inicio,
		  Fecha_Final: fecha_final,
		  Programa: programa,
		  Institucion: institucion,
  ...
  const elminarBloque = (plan_id, index) => {
		...
		manager_bloques.BorrarBloque(user_data, setUserData, "Educacion_Formal", plan_id);
        ...
        <Grid align="center" container spacing={0} className="wrapper">
          <div>
            <Paper style={paperStyle} sx={paperSX}>
              ...
              <List
                dense={dense}
                style={{ padding: "5px", maxHeight: "95%", overflow: "auto", backgroundColor: "#ccd5"}}
              >
				<ListItemButton key={true} style={listStyle, {backgroundColor: "#4f96"}} onClick={(e) => reiniciarForm()}>
				  <h2>+</h2>
				  <ListItemText
					primary={
					  "Agregar Nuevo"
